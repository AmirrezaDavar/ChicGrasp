...
Run with the original robodiff environment. The large checkpoint stays in memory
because a full extraction would exceed the presentation workstation's free disk.
"""
...
def main():
    ...
    ap.add_argument('--output', type=Path, default=ROOT/'docs/assets/data')
    args = ap.parse_args()
    ...
    # Preserve the checkpoint's trained n_action_steps. This is an offline
    # explanation, not an attempt to reproduce deployment timing overrides.
    ...
    def recorded_step(model_output, timestep, sample, **kwargs):
        if not current:
            current.append(sample.detach().cpu().numpy()[0].copy())
        result = original_step(model_output, timestep, sample, **kwargs)
        current.append(result.prev_sample.detach().cpu().numpy()[0].copy())
        return result
    ...
    for requested in args.times:
        ...
        tensors = {k: torch.from_numpy(v).unsqueeze(0).to(args.device) for k,v in obs_np.items()}
        current.clear()
    ...
    metadata = dict(kind='offline_checkpoint_replay', checkpoint_archive=archive.name,
                    ...
                    episode=args.episode, seed=42, inference_steps=args.steps,
