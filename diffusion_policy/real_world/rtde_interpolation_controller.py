...
############################################################################################################################################
        ...
        if receive_keys is None:
            receive_keys = [
                ...
                # 'LeftJawState', # added2 # results in error
                # 'RightJawState', # added3 # results in error
            ]
...
############################################################################################################################################
        ...
        ring_buffer = SharedMemoryRingBuffer.create_from_examples(
            shm_manager=shm_manager,
            examples=example, # here new values are######
            get_max_k=get_max_k,
            get_time_budget=0.2,
...
############################################################################################################################################
    ...
    def schedule_waypoint(self, pose, target_time):
        ...
        # pose = np.array(pose)
        # assert pose.shape == (6,)
        pose = np.array(pose)[:6]  # Only take the first 6 elements for the robot
        # assert pose.shape == (8,) # modified 

        message = {
            'cmd': Command.SCHEDULE_WAYPOINT.value,
            'target_pose': pose,
            # 'target_pose': target_pose[:6],  # Only the first 6 elements
...
############################################################################################################################################
            ...
            while keep_running:
                ...
                #### modify ######
                pose_command_robot = pose_command[:6]  # Only take the first 6 elements for the robot
                vel = 0.5
                acc = 0.5
                # assert rtde_c.servoL(pose_command,
                assert rtde_c.servoL(pose_command_robot,
